#!/usr/bin/env python3
"""
Fit Model 1: Baseline Engagement Model

This script:
1. Loads and prepares participant data
2. Fits Model 1 (baseline cost + savings model)
3. Runs diagnostics and quality checks
4. Saves results and generates plots

Usage:
    python src/bayesian_scripts/bayes_fit_model_01.py [--campaign-ids ID1,ID2,...]
"""
...
from src.bayesian_models.model_01_baseline import BaselineOpenModel


def parse_args():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(
        description='Fit Model 1: Baseline Engagement Model'
    )
    parser.add_argument(
        '--campaign-ids',
        type=str,
...
def main():
    ...
    print("\n" + "🔬 " * 20)
    print("MODEL 1: BASELINE ENGAGEMENT MODEL")
    print("🔬 " * 20 + "\n")
    ...
    # Save metadata
    import json
    metadata_path = output_dir / 'model_01_metadata.json'
    ...
    model_data = get_model_matrices(df, model_spec='baseline')
    ...
    # Trace plot
    trace_plot_path = output_dir / 'model_01_trace.png'
    ...
    posterior_plot_path = output_dir / 'model_01_posterior.png'
    model.plot_coefficients(save_path=str(posterior_plot_path))

    # Forest plot
    forest_plot_path = output_dir / 'model_01_forest.png'
    BayesianDiagnostics.plot_forest(
        trace,
        var_names=["beta_cost", "beta_savings"],
        save_path=str(forest_plot_path)
    )

    # Posterior predictive check
    ppc_plot_path = output_dir / 'model_01_ppc.png'
    ...
    trace_path = output_dir / 'model_01_trace.nc'
    model.save_trace(str(trace_path))

    # Save summary to CSV
    summary_df = diagnostics['coefficients']
    summary_path = output_dir / 'model_01_summary.csv'
    ...
    print("\n" + "✅ " * 20)
    print("MODEL 1 FITTING COMPLETE!")
    print("✅ " * 20 + "\n")
